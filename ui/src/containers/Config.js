...
const ConfigContainer = ({ localizationValue }) => {
  ...
  const blStartsWithKeyCharacter = (stringInput) =>
    stringInput.indexOf(`!`) === 0;
  ...
  const handleArrayLocalizedConfig = (arrayConfigInput) =>
    arrayConfigInput.map((item) => generateLocalizedConfig(item));
  ...
  const handleObjectLocalizedConfig = (objectConfigInput) => {
    try {
      ...
      Object.keys(objectConfigInput).map((item) => {
        const fieldNameToUse = blStartsWithKeyCharacter(item)
          ? getLocalizedValue(item)
          : item;
        return (returnObj[fieldNameToUse] = generateLocalizedConfig(
          objectConfigInput[item]
        ));
      });
      return returnObj;
    } catch (e) {
  ...
  return (
    <Fade in={true}>
      {requestError.length > 0 && (
        <Alert color="danger">{`${requestError}`}</Alert>
      )}
